...
volatile byte bitTimeCount = 0; //Counts the number of intterupts recived, places timing data into a array.
...
#define NEC_HEADER_PULSE_MIN 8500
...
void Decode(){
    if((micros()-lastDecodeRun >= 10000) && (bitTimeCount > 0)){
        lastDecodeRun = micros();
        int startindx = -1;
